...
class ClearTableCommand extends Command
{
		...
		foreach ($tableConfigs as $tableConfig) {
			...
			foreach ($dateRanges as $range) {
				...
				do {
					...
					[$start, $end] = $this->convertDateRange($range, $tableConfig);

					// 限制条件：日期范围 + id > lastId（无排序）
					$rows = $query
						->whereBetween($tableConfig['date_column'], [$start, $end])
						->where('id', '>', $lastId)
						->limit($chunkSize)
						->get(['id']);
	...
	private function convertDateRange(array $range, array $tableConfig): array
	{
		$type = $tableConfig['date_type'] ?? 'datetime';

		// 统一转成 Carbon（兼容你现在的 generateDateRanges）
